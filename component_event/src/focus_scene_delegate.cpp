...
FocusSceneDelegate::FocusSceneDelegate()
{
}
FocusSceneDelegate::~FocusSceneDelegate()
{
...
ErrCode FocusSceneDelegate::ChooseScene(bool isRandom)
{
    ErrCode result;
    GRID_COUNT = 0;
...
ErrCode FocusSceneDelegate::SetAvailableComponentList(std::shared_ptr<ComponentTree> componentinfo, bool isRandom)
{
    GRID_COUNT = 0;
    componentType_.clear();
    ErrCode result = OHOS::ERR_OK;
    ...
    if (isRandom) {
        ...
        normalscene.SetInputComponentListForFocusInput(componentlist);
...
ErrCode FocusSceneDelegate::FindSamePageInChildren(bool &isFound, bool isRandom)
{
    ErrCode result = OHOS::ERR_OK;
    auto treemanager = TreeManager::GetInstance();
    auto newpage = treemanager->GetNewPage();
...
ErrCode FocusSceneDelegate::FindSamePageInParent(bool &isFound, bool isRandom)
{
    ErrCode result = OHOS::ERR_OK;
    auto treemanager = TreeManager::GetInstance();
    auto newpage = treemanager->GetNewPage();
...
bool FocusSceneDelegate::IsComponentInScreen(const std::shared_ptr<ComponentTree> componentinfo)
{
    int32_t width = -1;
    int32_t height = -1;
...
bool FocusSceneDelegate::IsPageChanged()
{
    auto treemanager = TreeManager::GetInstance();
    auto newpage = treemanager->GetNewPage();
    if (newpage == nullptr) {
        DEBUG_LOG("newpage is nullptr");
        return false;
    }
    // get current page node
    std::shared_ptr<WuKongTree> currentpage = treemanager->GetCurrentPage();
